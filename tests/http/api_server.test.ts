...
function makeEvent(overrides: Partial<AcediaEvent> = {}): AcediaEvent {
    return {
        ...
        title: "Test", priority: "normal", dedupeKey: `e-${Math.random()}`,
        ...overrides,
    };
}

function get(url: string, headers: Record<string, string> = {}): Promise<{ status: number; body: unknown }> {
...
function makeConnector(name: string, executeAction?: (a: ConnectorAction) => Promise<void>): IConnector {
...
describe("AcediaApiServer — POST /api/actions", () => {
    it("should dispatch action to matching connector", async () => {
        ...
        const store    = new EventStore();
        let called     = false;
        const conn     = makeConnector("Gmail", async () => { called = true; });
        const server   = new AcediaApiServer(store, [conn], null, SECRET);
        server.start(port);
        const res = await post(`http://localhost:${port}/api/actions`,
            { connector: "Gmail", action: { kind: "reply", sourceId: "msg1", body: "Hi" } }, AUTH);
        server.stop();
    ...
    it("should return 404 when connector not found", async () => {
        ...
        server.start(port);
        const res = await post(`http://localhost:${port}/api/actions`,
            { connector: "NoSuchConnector", action: { kind: "complete", sourceId: "x" } }, AUTH);
        server.stop();
        expect(res.status).toBe(404);
    });
    ...
    it("should return 400 when connector does not support actions", async () => {
        ...
        server.start(port);
        const res = await post(`http://localhost:${port}/api/actions`,
            { connector: "ReadOnly", action: { kind: "complete", sourceId: "x" } }, AUTH);
        server.stop();
...
describe("AcediaApiServer — /api/devices/push-token", () => {
    it("should return 503 when FCM is not configured", async () => {
        ...
        const server = new AcediaApiServer(store, [], null, SECRET);
        server.start(port);
        const res = await post(`http://localhost:${port}/api/devices/push-token`, { token: "abc" }, AUTH);
        server.stop();
